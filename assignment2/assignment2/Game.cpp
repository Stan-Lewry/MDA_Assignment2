		...
		for (int j = 0; j < mapW; j++){
			...
			if (clickX > map[i][j].screenX && clickX < map[i][j].screenX + tileSize){
				if (clickY > map[i][j].screenY && clickY < map[i][j].screenY + tileSize){	
					...
					checkMovementRange(4, map[i][j].worldX, map[i][j].worldY);
...
bool Game::isTraversable(int x, int y){
	if (x >= 0){
		if (x < mapW){
			if (y >= 0){
				if (y < mapH){
...
void Game::checkMovementRange(int moveDist, int originX, int originY){
...
void Game::renderMap(){
	...
	for (int i = 0; i < mapH; i++){
		for (int j = 0; j < mapW; j++){
			...
			else if (map[i][j].moveRange == true){
				sRect.y = 9 * 32;
				SDL_RenderCopy(sdlUtils->rend, mapTileTexture, &sRect, &dRect);
			}
		}
